...
export default function AdminQnaComment() {
    ...
    useEffect(() => {
        axios.post('http://15.164.97.230:9001/chatbot/detail', { "qid": qid })
            .then(res => {
                setFormData(res.data);

            })
            .catch(err => console.log(err));
        axios.post('http://15.164.97.230:9001/chatbot/getCustomerInfo', { id })
            .then(res => {
    ...
    const handleComment = () => {
        if(validate()){
            axios.post('http://15.164.97.230:9001/chatbot/updateComment', { "no": formData.no, inputData })
            .then(res => {
